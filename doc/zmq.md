...
The Sallmon Core daemon can be configured to act as a trusted "border
router", implementing the sallmon wire protocol and relay, making
...
The ZeroMQ feature in Sallmon Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building sallmond:
...
For instance:
    ...
    $ sallmond -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/sallmond.tx.raw
...
These options can also be provided in sallmon.conf.
...
From the perspective of sallmond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into sallmond directly. Furthermore, no information is
...
using. Sallmond appends an up-counting sequence number to each
