...
#include "sallmonaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Sallmon address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new SallmonAddressEntryValidator(parent));
    widget->setCheckValidator(new SallmonAddressCheckValidator(parent));
}
...
bool parseSallmonURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no sallmon: URI
    if(!uri.isValid() || uri.scheme() != QString("sallmon"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SallmonUnits::parse(SallmonUnits::RVN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSallmonURI(QString uri, SendCoinsRecipient *out)
{
    // Convert sallmon:// to sallmon:
    //
    //    Cannot handle this later, because sallmon:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sallmon://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "sallmon:");
    }
    QUrl uriInstance(uri);
    return parseSallmonURI(uriInstance, out);
}

QString formatSallmonURI(const SendCoinsRecipient &info)
{
    QString ret = QString("sallmon:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SallmonUnits::format(SallmonUnits::RVN, info.amount, false, SallmonUnits::separatorNever));
        paramCount++;
    }
...
bool openSallmonConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(SALLMON_CONF_FILENAME);
    ...
    /* Open sallmon.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sallmon.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sallmon (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Sallmon (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Sallmon*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "sallmon.desktop";
    return GetAutostartDir() / strprintf("sallmon-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a sallmon.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Sallmon\n";
        else
            optionFile << strprintf("Name=Sallmon (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the sallmon app
...
bool GetStartOnSystemStartup()
{
    CFURLRef sallmonAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (sallmonAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, sallmonAppUrl);

    CFRelease(sallmonAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add sallmon app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, sallmonAppUrl, nullptr, nullptr);
    }
