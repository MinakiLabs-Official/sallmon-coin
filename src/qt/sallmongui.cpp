...
// Copyright (c) 2017-2021 The Sallmon Core developers
...
#include "sallmongui.h"

#include "sallmonunits.h"
...
const std::string SallmonGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SallmonGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(SallmonGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

SallmonGUI::SallmonGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void SallmonGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Sallmon address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and sallmon: URIs)"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Sallmon addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Sallmon addresses"));
    ...
    openAction->setStatusTip(tr("Open a sallmon: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Sallmon command-line options").arg(tr(PACKAGE_NAME)));
...
void SallmonGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            ...
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/sallmoncointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* sallmonLabelLayout = new QVBoxLayout(toolbarWidget);
        sallmonLabelLayout->addWidget(labelToolbar);
        sallmonLabelLayout->addWidget(m_toolbar);
        sallmonLabelLayout->setDirection(QBoxLayout::TopToBottom);
        sallmonLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Sallmoncoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/SallmonProject/Sallmoncoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Sallmoncoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/SallmonProject/Sallmoncoin/releases";
...
void SallmonGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/rvntext")));
        m_toolbar->setMaximumWidth(65);
...
void SallmonGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SallmonGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SallmonGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SallmonGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SallmonGUI::setWalletActionsEnabled(bool enabled)
{
...
void SallmonGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SallmonGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SallmonGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SallmonGUI::optionsClicked()
{
...
void SallmonGUI::showDebugWindow()
{
...
void SallmonGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SallmonGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void SallmonGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SallmonGUI::openClicked()
{
...
void SallmonGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SallmonGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SallmonGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SallmonGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SallmonGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SallmonGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RVN START */
void SallmonGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void SallmonGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void SallmonGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void SallmonGUI::gotoRestrictedAssetsPage()
{
...
void SallmonGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Sallmon network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void SallmonGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SallmonGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SallmonGUI::updateHeadersSyncProgressLabel()
{
...
void SallmonGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SallmonGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Sallmon"); // default title
...
void SallmonGUI::changeEvent(QEvent *e)
{
...
void SallmonGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SallmonGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SallmonGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RVN")
        msg += tr("Amount: %1\n").arg(SallmonUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(SallmonUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void SallmonGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SallmonGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SallmonGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SallmonGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SallmonGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void SallmonGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SallmonGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SallmonGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SallmonGUI::detectShutdown()
{
...
void SallmonGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SallmonGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SallmonGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(SallmonGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void SallmonGUI::subscribeToCoreSignals()
{
...
void SallmonGUI::unsubscribeFromCoreSignals()
{
...
void SallmonGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SallmonGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SallmonUnits::Unit> units = SallmonUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SallmonUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(SallmonUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(SallmonUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (SallmonUnits::Unit u : SallmonUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SallmonUnits::name(u)), this);
...
{
    setText(SallmonUnits::name(newUnits));
}
...
void SallmonGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SallmonGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "SallmonGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void SallmonGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/SallmonProject/Sallmoncoin/releases"));
