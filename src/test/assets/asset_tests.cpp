...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of SALLMONCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("RVN", type));
        BOOST_CHECK(!IsAssetNameValid("SALLMON", type));
        BOOST_CHECK(!IsAssetNameValid("SALLMONCOIN", type));

        //- Versions of SALLMONCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("SALLMON.COIN", type));
        BOOST_CHECK(IsAssetNameValid("SALLMON_COIN", type));
        BOOST_CHECK(IsAssetNameValid("RVNSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERRVN", type));
        BOOST_CHECK(IsAssetNameValid("SALLMONSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDESALLMON", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_SALLMONS", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("SALLMON", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("SALLMON", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
