...
The RPM spec file provided here is for Sallmon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sallmon.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sallmon git repository](https://github.com/SallmonProject/Sallmoncoin) but are not part of
...
Currently only `Source10` is used, for the example `sallmon.conf` file.
...
in the sallmon git repository. At present only an SVG version of the Sallmon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sallmon
...
needed when 0.12.1 is released, a proper fix is already in the Sallmon git
master branch.
...
idea to build Sallmon Core against a static version of that library compiled
...
The `sallmon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sallmon-server` package. At this point, most RPM based
...
The `sallmon-server` package should have SELinux support. How to properly do
...
Sallmon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sallmon/). He has been
packaging Sallmon for Fedora at least since 2012.
...
4. I renamed the `sallmon` package that contains the Qt GUI to `sallmon-core` as
that appears to be how the general population refers to it, in contrast to
`sallmon-xt` or `sallmon-classic`. I wanted to make sure the general population
