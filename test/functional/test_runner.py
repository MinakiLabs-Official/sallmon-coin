...
`test/functional/test_framework/test_framework.py:SallmonTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/sallmon_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and sallmond
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_sallmond = config["components"].getboolean("ENABLE_SALLMOND")
    if not (enable_wallet and enable_cli and enable_sallmond):
        print("No functional tests to run. Wallet, utils, and sallmond must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if sallmond is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "sallmond"]) is not None:
            print("%sWARNING!%s There is already a sallmond process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SALLMOND" not in os.environ:
        os.environ["SALLMOND"] = build_dir + '/src/sallmond' + exeext
        os.environ["SALLMONCLI"] = build_dir + '/src/sallmon-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `sallmon-cli help` (`rpc_interface.txt`).
